...
def set_to_str(args_dict, param_set):
    return "_".join(["_".join([k, str(v)]) for k, v in args_dict.items() if k in param_set])


def feature_concat(s_dsnae, t_dsnae, s_batch, t_batch, device):
    source_shared_codes = s_dsnae.s_encode(s_batch[0].to(device))
    target_private_codes = t_dsnae.p_encode(t_batch[0].to(device))
    s_y = s_batch[1]
    ...
    return t_dsnae.decoder(concat_feature).detach(), s_y
...
def load_classifier(task_folder_path, fold_count, **kwargs):
    ...
    target_classifier.load_state_dict(torch.load(os.path.join(task_folder_path, f'classifier_{fold_count}.pt'), map_location=torch.device(kwargs['device'])))
...
def generate_disyn(**kwargs):
    ...
    s_dsnae = DiSyn(shared_encoder=shared_encoder,
    ...
    t_dsnae = DiSyn(shared_encoder=shared_encoder,
                    adapter=shared_encoder_linker,
                    decoder=shared_decoder,
                    input_dim=kwargs['input_dim'],
                    latent_dim=128,
    ...
    return s_dsnae, t_dsnae


def build_disyn_from_classifier(task_folder_path, fold_count, **kwargs):
    s_dsnae, t_dsnae = generate_disyn(**kwargs)

    target_classifier = load_classifier(task_folder_path, fold_count, **kwargs)

    s_dsnae.shared_encoder.load_state_dict(target_classifier.encoder.state_dict())
    t_dsnae.shared_encoder.load_state_dict(target_classifier.encoder.state_dict())
    s_dsnae.adapter.load_state_dict(target_classifier.decoder.module.state_dict())
    t_dsnae.adapter.load_state_dict(target_classifier.decoder.module.state_dict())

    assert (id(s_dsnae.shared_encoder) == id(t_dsnae.shared_encoder))
    assert (id(s_dsnae.adapter) == id(t_dsnae.adapter))
...
def load_classifier_for_inference(model_path, **kwargs):
    shared_encoder = FC(input_dim=kwargs['input_dim'],
                        output_dim=128,
