...
def data_synthesize(s_dsnae, t_dsnae, train_dataloader, test_dataloader, di_scale_co, device):
    # ##################################################################################################################
    # ################################################## Di synthesis ##################################################
    ...
    for i in range(di_scale_co):
        for step, s_batch in enumerate(train_dataloader):
            ...
            # If the numbers of the two batches are different, the alignment will be supplemented first.
            if len(s_x) > len(t_x):
            ...
            source_shared_codes = s_dsnae.s_encode(s_x)
            target_private_codes = t_dsnae.p_encode(t_x)
            concat_feature = torch.cat((target_private_codes, source_shared_codes), dim=1)
            synsample = t_dsnae.decode(concat_feature).detach()
...
def disyn_train_step_backward(critic, s_dsnae, t_dsnae, s_batch, t_batch, device, optimizer, alpha, loss_function, scheduler=None):
    critic.zero_grad()
    ...
    s_loss = dsn_loss(*s_dsnae(s_x))
    t_loss = dsn_loss(*t_dsnae(t_x))
    recons_loss = s_loss + t_loss
...
def class_adv_train_step(critic, s_dsnae, t_dsnae, s_batch, t_batch, device, optimizer, loss_function, scheduler=None):
    ...
    s_code = s_dsnae.p_encode(s_x)
    t_code = t_dsnae.p_encode(t_x)
...
def task_specific_train_di_adv(source_dsn, source_labeled_train_dataloader, source_labeled_val_dataloader, target_test_dataloader, fold_count, metrics_dict, **kwargs):
    """
    task_specific_train ft function
    """
    ...
    for epoch in range(2000):
        if epoch % 50 == 0:
            print(f'classification train epoch {epoch}')

        for step, s_batch in enumerate(source_labeled_train_dataloader):
            classification_train_step(model=target_classifier, batch=s_batch, loss_fn=classification_loss, device=kwargs['device'],
        ...
        # Part of implementations is from https://github.com/XieResearchGroup/CODE-AE/blob/6dc17a5f3b7ce2e89736d1d575fb75951bd2c9ea/code/fine_tuning.py#L85.
...
def recon_classadv_train_di_adv(dsn_source, dsn_target, s_labeled_dataloader, t_unlabeled_dataloader, fold_count, **kwargs):
    ...
    for epoch in range(kwargs['clf_train_bf_recon']):
        for step, s_batch in enumerate(s_labeled_dataloader):
            ...
            class_adv_train_step(critic=class_confounding_classifier, s_dsnae=dsn_source, t_dsnae=dsn_target, s_batch=s_batch, t_batch=t_batch, device=kwargs['device'],
    ...
    os.makedirs(os.path.join(kwargs['task_save_path'], f'recon_epochs_{kwargs["recon_epochs"]}'), exist_ok=True)
    torch.save(dsn_source.state_dict(), os.path.join(kwargs['task_save_path'], f'recon_epochs_{kwargs["recon_epochs"]}', f's_disyn_adv_recon_{fold_count}.pt'))
    torch.save(dsn_target.state_dict(), os.path.join(kwargs['task_save_path'], f'recon_epochs_{kwargs["recon_epochs"]}', f't_disyn_adv_recon_{fold_count}.pt'))
...
def task_specific_train_disyn_step2(dsn_source, dsn_target, s_labeled_train_dataloader, s_labeled_val_dataloader, t_unlabeled_dataloader,
    ...
    torch.save(target_classifier.state_dict(), os.path.join(kwargs['task_save_path'], f'di_adv_classifier_{fold_count}.pt'))
    ...
    for epoch in range(2000):
        ...
        for step, synthe_batch in enumerate(synthe_dataloader):
            ...
            #
            #     dsn_recon_step(s_dsnae=s_dsnae,
            #                    t_dsnae=t_dsnae,
    ...
    metrics_dict['best_index'].append(val_history['best_index'])
    # 记录的是 val_set 上的最优的那个点的 test_set 结果
    for metric in ['auroc', 'acc', 'aps', 'f1', 'auprc']:
        metrics_dict[metric].append(test_history[metric][val_history['best_index']])
